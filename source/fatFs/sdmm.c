...
void dly_us(int n)
{
	while(n--)
	{
		for(volatile int i = 0; i < CYCLESUS; i++);
...
)
{
    ...
    CS_H();
    /* Initialize port pin tied to SCLK */
    PORT2->PCR[12] = PORT_PCR_LK(1);
    CK_L();
